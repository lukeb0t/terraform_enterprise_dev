...
Deploys Terraform Enterprise (disk mode) on a single Ubuntu 22.04 EC2 instance using Docker Compose. TFE is accessed directly by its Elastic IP address. A self-signed certificate is generated by default; you can supply your own cert via the `tls_*` variables.
